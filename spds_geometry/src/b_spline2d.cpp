#include <spds_geometry/b_spline2d.h>
...
void
BSpline2d::setPoses(const spds_generic::Pose2dContainerInterface &poses, double speed, double knotOffset)
{
     clear();
...
void 
BSpline2d::setPosesAsKnots(const spds_generic::Pose2dContainerInterface &poses, double speed, double knotOffset)
{
  clear();
...
void
BSpline2d::addPose(const spds_generic::Pose2d &pose, double speed, double knotOffset)
{
     std::vector<double> x;
     std::vector<double> y;
...
void
BSpline2d::addKnot(const spds_generic::Pose2d &pose, double speed)
{
  assert(_x.size() == _y.size());
  assert(_x.size() == _speed.size());
...
void 
BSpline2d::calcKnots(const spds_generic::Pose2d &pose, std::vector<double> &x, std::vector<double> &y, double offset)
{
  double mx = pose(0);
  double my = pose(1);
  double mth = pose(2);
...
void 
BSpline2d::getKnots(spds_generic::PositionVec &pos) const
{
...
//     std::cout << "x : " << x << " y : " << y << " dx : " << dx << " dy : " << dy << std::endl;
     if (getSpeed(t) >= 0)
       return spds_generic::Pose2d(x, y, atan2(dy, dx));
     else
       return spds_generic::Pose2d(x, y, angles::normalize_angle(atan2(dy, dx)+M_PI));
}
...
bool 
BSpline2d::collision(const spds_generic::CollisionCheckInterface &collisionCheck, double resolution) const
{
     double min_T = getT(1);
...
void 
BSpline2d::calcWaypoints(spds_generic::Pose2dVec &points, double resolution) const
{
     points.clear();
...
void 
BSpline2d::calcPath(spds_generic::Path &path, double L, double resolution) const
{
  path.clear();
