...
#include <spds_geometry/geometry.h>

namespace spds_geometry {

  class Polygon : public spds_generic::Point2dContainerInterface, spds_generic::Point2dCollisionCheckInterface  {
  public:
    Polygon();
    ...
    bool intersection(const spds_geometry::Polygon &p) const;

    Eigen::Vector2d getCenter() const;

    void clear();

    Polygon getMovedPolygon(const spds_generic::Pose2d &pose) const {
      return Polygon(getMovedPoint2dContainer(*this, pose));
    }

    spds_generic::Point2dVec points;
  /* private: */
