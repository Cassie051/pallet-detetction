...
namespace spds_geometry {
  ...
  inline void movePoint2dContainer(spds_generic::Point2dContainerInterface &pts, const spds_generic::Pose2d &pose) {
    Eigen::Vector2d offset = spds_generic::getPosition(pose);
    Eigen::Matrix2d rot;
  ...
  inline spds_generic::Point2dVec getMovedPoint2dContainer(const spds_generic::Point2dContainerInterface &pts, const spds_generic::Pose2d &pose) {
    spds_generic::Point2dVec ret(pts);
    movePoint2dContainer(ret, pose);
    return ret;
  }

  inline double getMinDistanceFromOrigin(const spds_generic::Point2dContainerInterface &pts) {
  ...
  inline double getMaxDistanceFromOrigin(const spds_generic::Point2dContainerInterface &pts) {
    double d = 0;
    for (size_t i = 0; i < pts.sizePoint2d(); i++) {
      const Eigen::Vector2d &v = pts.getPoint2d(i);
      if (v.dot(v) > d)
  ...
  inline double minDistanceSqr(const spds_generic::Point2dContainerInterface &pts) {
  ...
  inline spds_generic::Point2dVec getMinDistanceBetweenPoint2dVec(const spds_generic::Point2dContainerInterface &pts, double minDist) {
    spds_generic::Point2dVec ret;
    size_t pts_size = pts.sizePoint2d();
    if (pts_size < 1)
      return ret;
  ...
  inline void calculateMatrixForm(const spds_generic::Point2dContainerInterface &pts_, Eigen::MatrixXd &A, Eigen::VectorXd &b) {
    ...
    spds_generic::Point2dVec pts = spds_geometry::getMinDistanceBetweenPoint2dVec(pts_, min_dist);
  ...
  inline Eigen::Vector2d getCenter(const spds_generic::Point2dContainerInterface &pts) {
    size_t pts_size = pts.sizePoint2d();
  ...
  inline void getBoundingBox2d(const spds_generic::Point2dContainerInterface &pts, Eigen::Vector2d &bottomLeft, Eigen::Vector2d &topRight) {
    size_t pts_size = pts.sizePoint2d();
    assert(pts_size != 0);
