...
#include <spds_geometry/b_spline.h>
#include <spds_generic/pose2d.h>
...
public:
  ...
  public:
	...
	return os;
     ...
     void setPoses(const spds_generic::Pose2dContainerInterface &poses, double speed, double knotOffset = 1.);
     void setPosesAsKnots(const spds_generic::Pose2dContainerInterface &poses, double speed, double knotOffset = 1.);

     // Add a single pose creating 3 knots.
     void addPose(const spds_generic::Pose2d &pose, double speed, double knotOffset);
     // Add a single pose extracting only the position (x,y) to the knot.
     void addKnot(const spds_generic::Pose2d &pose, double speed);
...
//     void setPoses(const std::vector<lsl::lslPose3d> &poses, std::vector<double> speeds);
     ...
     void getKnots(spds_generic::PositionVec &pos) const;

     //! Return a pose at the value \param t.
     spds_generic::Pose2d getPose(double t) const;
     ...
     void calcWaypoints(spds_generic::Pose2dVec &points, double resolution) const;

     void calcPath(spds_generic::Path &path, double L, double resolution) const;

     spds_generic::Pose2d getGoal() const;
     ...
     bool collision(const spds_generic::CollisionCheckInterface &collisionCheck, double resolution) const;
     bool evaluate(BSpline2d::Evaluation &eval, double L, double minSteeringAngle, double maxSteeringAngle, double resolution) const;


private:
     void calcKnots(const spds_generic::Pose2d &pose, std::vector<double> &x, std::vector<double> &y, double offset);
     BSpline _x;
     BSpline _y;
     BSpline _speed;
