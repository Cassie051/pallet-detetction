...
namespace spds_generic
{
     ...
     public:
	  ...
	  Pose2d mean;
	  Eigen::Matrix3d cov;
	  friend std::ostream& operator<<(std::ostream &os, const spds_generic::Pose2dCov &obj)
	  {
	       os << "\n[mean] : \n" << obj.mean;
	       os << "\n[cov ] : \n" << obj.cov;
	       return os;
     ...
     inline bool addStepPose2d(const spds_generic::Pose2d &p1, const spds_generic::Pose2d &p2, double maxDist, double maxRotation) {
...
inline spds_generic::Pose2d pose2dFromAffine3d(const Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor> &T) {
  Pose2d ret;
...
inline void pose2dClearDependence(spds_generic::Pose2dCov &posecov) {
  posecov.cov(0,1) = 0.;
  posecov.cov(1,0) = 0.;
  posecov.cov(0,2) = 0.;
  posecov.cov(1,2) = 0.;
