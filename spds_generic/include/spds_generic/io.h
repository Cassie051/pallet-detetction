...
namespace spds_generic
{
  ...
  inline std::vector<spds_generic::Path> loadPathDirTextFile(const std::string &dirName) {
	...
	spds_generic::Path p = spds_generic::loadPathTextFile(file_names[i]);
	if (p.sizePath() == 0)
	  {
  ...
  inline void saveQuaternionPathTextFile(const PathInterface &path, const std::string &fileName) { 
    ...
    for (unsigned int i = 0; i < path.sizePath(); i++)
      {
	Eigen::Quaterniond q = spds_generic::getQuaterion(path.getPose2d(i));
	ofs << path.getPose2d(i)(0) << " "
	    << path.getPose2d(i)(1) << " "
	    << q.z() << " "
	    << q.w() << " "
  ...
  inline void saveTraversedDistanceTrajectoryTextFile(const TrajectoryInterface &traj, const std::string &fileName) {
	...
	dist_sum += spds_generic::getDistBetween(traj.getPose2d(i), traj.getPose2d(i+1));
      }
    }	  
  ...
  inline CoordinatedTimes loadCoordinatedTimesTextFile(const std::string &fileName) {
    std::vector<double> tmp = spds_generic::loadDoubleVecTextFile(fileName);
