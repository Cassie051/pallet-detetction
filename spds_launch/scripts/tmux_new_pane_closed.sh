...
if tmux has -t ros:$window_name 2>/dev/null; then
  # if has then
  # Open new pane in specified window, set pane name,
  # set layout and roslaunch file
  tmux switch -t ros \; split-window -h -t $window_name "${*} __ns:=$ROS_NAMESPACE; bash -i" \; \
  select-pane -T ${pane_name} \; select-layout tiled \; last-window \; 
else
